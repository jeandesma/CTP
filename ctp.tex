...
In order to have a better view of what $\phi(K,L)$ represents, we propose to the reader to look at [Figure 1]:
...
Furthermore,in order to have a better view of the long formula for $T_{fd}$ and $T_{fa}$ with $P_0$, we can look into [Figure 2].

We can see from [Figure 2a] that if we fix K, the performance improves quickly with the growth of L, and tends to optimal.
And from [Figure 2b] we know that if we set L=1 for just one traveler, the result will be $K+1$, which correspond with the result of Bender et al.~\cite{BeWe15};when there are multiple travellers (L=20 for example), the performance depends on the number of blockages, and usually when the number of blockages is less than or similar with the number of travellers, the performance is nearly optimal. But we also  find that with the increase of K ,the performance begin to drop faster and faster. So when K reaches a number which is big enough, the drop of the performance will no longer be negligible.
...
The numerical results could be useful in many applications. For example, by studying the distance between $T_{fd}$ and $T_{fa}$, we can judge whether a team of travellers can be more efficient in finding the target:

(1)With deterministic approach, the cost is $2K+1$ with one traveller, so it makes no difference to use multiple travellers when there is no communication($P_0$). However, with partial communication or complete communication, the more travellers we have, the better result we will get. Because the formula of the cost is $2(K+1)-L$ which concerns about the number of travellers, so if we want this result less than $2K+1$, we should have $L>1$.

(2)With randomized approach, the cost becomes $K+1$ for only one traveller. When there is no communication, we can compare the formula with $K+1$. Unless K is enormously big, multiple travellers will be a better choice. With complete communication, if we want the result better than $K+1$ which is the case of one traveller, we should have the following condition:
\[
(K+1)-(K+2-L+\frac{L(L-1)}{2(K+1)})>0
\]
so when $L>1$ and $2K+2>L$, multiple travellers can be better than only one traveller.

For future work, we recommend to explore the possibility of establishing a bi-criteria that would include both distance and time. This would expand the direct applications of the K-Canadian travellers problem for a fleet of vehicles that wants to reach destination quickly without spending to much on gas. 
